...
import { HUNTER_RANKS, type HunterRank } from '@/lib/xp';

interface Props {
  rank: HunterRank | undefined;
  nextRank: HunterRank | null | undefined;
  score: number;
  progressPct: number;
  toNext: number;
  subtitle?: string;
...
export function QuestRankBadgeCard({
  rank,
  nextRank,
  score,
  progressPct,
  ...
  const character = rank?.key ? HUNTER_CHARACTER_BY_KEY[rank.key] : undefined;
  const levelIdx = rank ? HUNTER_RANKS.findIndex((r) => r.key === rank.key) : -1;
  const level = levelIdx >= 0 ? levelIdx + 1 : null;
  const Wrap: any = onPress ? Pressable : View;

  return (
    <Wrap onPress={onPress} style={[styles.card, { borderColor: QUEST_ACCENT + '55', height: 210 }]}>
      <LinearGradient
        colors={['rgba(20,7,30,0.95)', 'rgba(7,12,20,0.95)']}
        start={{ x: 0, y: 0 }}
        end={{ x: 1, y: 1 }}
      ...
      <View style={styles.hero}>
        ...
        <View style={styles.info}>
          <View style={styles.headerRow}>
            ...
            <Text style={[styles.metaValue, { color }]}>{score.toLocaleString()} clears</Text>
          </View>
      ...
      <View style={styles.body}>
        ...
        <View style={styles.metaRow}>
          <View style={styles.metaCol}>
            <Text style={styles.metaLabel}>Clears</Text>
          ...
          <View style={styles.metaCol}>
            ...
            <Text style={[styles.metaValue, { color }]}>
              {nextRank ? `${toNext.toLocaleString()} clears` : 'MAXED'}
            </Text>
          </View>
        </View>
