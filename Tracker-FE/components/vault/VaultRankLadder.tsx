...
export function VaultRankLadder({ ladder, score, currentKey, unitLabel }: Props) {
  return (
    <View style={styles.list}>
      {ladder.map((r, idx) => {
        ...
        const level = idx + 1;
        const character = MAGE_CHARACTER_BY_KEY[r.key];
        const color = r.color;
        ...
        return (
          <View
            ...
            style={[
              styles.card,
              { borderColor: unlocked ? VAULT_GOLD + '55' : palette.border },
              isCurrent && {
                borderColor: color,
                shadowColor: color,
                shadowOpacity: 0.55,
            ...
            <LinearGradient
              colors={
                unlocked
                  ? ['rgba(30,22,7,0.95)', 'rgba(16,12,7,0.95)']
            ...
            <View style={styles.row}>
              ...
              <View style={styles.info}>
                <Text
                  style={[styles.level, { color: unlocked ? color : vaultText.tertiary }]}>
                  LEVEL {String(level).padStart(2, '0')}
                </Text>
                <Text
                  style={[
                    styles.name,
                    { color: unlocked ? color : vaultText.secondary },
                  ]}
                  numberOfLines={2}>
                    ...
                    {character.points.map((p) => (
                      <View key={p} style={styles.pointRow}>
                        <View
                          style={[
                            styles.dot,
                            { backgroundColor: unlocked ? color : vaultText.faint },
                          ]}
                        />
                        <Text
                          style={[
                            styles.pointTxt,
                            !unlocked && { color: vaultText.tertiary },
                          ]}
                          numberOfLines={1}>
                          {p}
                        </Text>
