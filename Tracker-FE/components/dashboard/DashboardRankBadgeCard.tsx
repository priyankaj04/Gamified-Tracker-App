...
import { dashboardText, palette, screenTheme } from '@/lib/themes';
import { SOLO_LEVELING_CHARACTER_BY_KEY } from '@/lib/soloLevelingBadges';
import {
  SOLO_LEVELING_RANKS,
  getDashboardRank,
  nextDashboardRank,
...
const DASHBOARD_ACCENT = screenTheme.dashboard.accent;
...
export function DashboardRankBadgeCard({
  totalXp,
  subtitle,
  accent,
  onPress,
  ...
  const rank: ModuleRank = getDashboardRank(totalXp);
  const nextRank = nextDashboardRank(totalXp);
  const color = rank.color ?? accent ?? DASHBOARD_ACCENT;
  const character = SOLO_LEVELING_CHARACTER_BY_KEY[rank.key];
  const levelIdx = SOLO_LEVELING_RANKS.findIndex((r) => r.key === rank.key);
  const level = levelIdx >= 0 ? levelIdx + 1 : null;
  const progressPct = nextRank
    ? Math.min(100, Math.round(((totalXp - rank.min) / (nextRank.min - rank.min)) * 100))
    : 100;
  const toNext = nextRank ? Math.max(0, nextRank.min - totalXp) : 0;
  const Wrap: any = onPress ? Pressable : View;

  return (
    <Wrap
      onPress={onPress}
      style={[styles.card, { borderColor: DASHBOARD_ACCENT + '55', height: 210 }]}>
      <LinearGradient
        colors={['rgba(18,7,30,0.95)', 'rgba(7,7,20,0.95)']}
        start={{ x: 0, y: 0 }}
        end={{ x: 1, y: 1 }}
      ...
      <View style={styles.hero}>
        ...
        <View style={styles.info}>
          <View style={styles.headerRow}>
            ...
            <Text style={[styles.metaValue, { color }]}>{totalXp.toLocaleString()} XP</Text>
          </View>
      ...
      <View style={styles.body}>
        ...
        <View style={styles.metaRow}>
          <View style={styles.metaCol}>
            ...
            <Text style={[styles.metaValue, { color }]}>{totalXp.toLocaleString()}</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  artWrap: { width: 180, alignSelf: 'stretch', overflow: 'hidden' },
  art: { width: '100%', height: '100%' },
  heroRight: { position: 'absolute', top: 12, right: 12 },
  body: {
  ...
  sub: { color: dashboardText.secondary, fontSize: 11, fontWeight: '700' },
  ...
  barTxt: { color: dashboardText.secondary, fontSize: 11, fontWeight: '800' },
  cta: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 4,
