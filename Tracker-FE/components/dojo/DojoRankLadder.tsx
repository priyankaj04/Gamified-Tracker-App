...
export function DojoRankLadder({ ladder, score, currentKey, unitLabel }: Props) {
  return (
    <View style={styles.list}>
      {ladder.map((r, idx) => {
        ...
        const level = idx + 1;
        const character = DEMON_SLAYER_CHARACTER_BY_KEY[r.key];
        const color = r.color;
        ...
        return (
          <View
            ...
            style={[
              styles.card,
              { borderColor: unlocked ? DOJO_FLAME + '55' : palette.border },
              isCurrent && {
                borderColor: color,
                shadowColor: color,
                shadowOpacity: 0.55,
            ...
            <LinearGradient
              colors={
                unlocked
                  ? ['rgba(30,12,7,0.95)', 'rgba(20,7,7,0.95)']
            ...
            <View style={styles.row}>
              ...
              <View style={styles.info}>
                <Text
                  style={[styles.level, { color: unlocked ? color : dojoText.tertiary }]}>
                  LEVEL {String(level).padStart(2, '0')}
                </Text>
                <Text
                  style={[
                    styles.name,
                    { color: unlocked ? color : dojoText.secondary },
                  ]}
                  numberOfLines={2}>
                    ...
                    {character.points.map((p) => (
                      <View key={p} style={styles.pointRow}>
                        <View
                          style={[
                            styles.dot,
                            { backgroundColor: unlocked ? color : dojoText.faint },
                          ]}
                        />
                        <Text
                          style={[
                            styles.pointTxt,
                            !unlocked && { color: dojoText.tertiary },
                          ]}
                          numberOfLines={1}>
                          {p}
                        </Text>
