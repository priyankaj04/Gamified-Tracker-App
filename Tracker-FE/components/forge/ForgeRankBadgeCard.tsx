...
import { forgeText, palette, screenTheme } from '@/lib/themes';
import { BLUE_LOCK_CHARACTER_BY_KEY } from '@/lib/bluelockBadges';
import { BLUE_LOCK_RANKS, type ModuleRank } from '@/lib/xp';
...
const FORGE_CYAN = screenTheme.forge.accent;
...
export function ForgeRankBadgeCard({
  rank,
  nextRank,
  score,
  progressPct,
  ...
  const character = rank?.key ? BLUE_LOCK_CHARACTER_BY_KEY[rank.key] : undefined;
  const levelIdx = rank ? BLUE_LOCK_RANKS.findIndex((r) => r.key === rank.key) : -1;
  const level = levelIdx >= 0 ? levelIdx + 1 : null;
  const Wrap: any = onPress ? Pressable : View;

  return (
    <Wrap onPress={onPress} style={[styles.card, { borderColor: FORGE_CYAN + '55', height: 210 }]}>
      <LinearGradient
        colors={['rgba(7,18,30,0.95)', 'rgba(7,12,20,0.95)']}
        start={{ x: 0, y: 0 }}
        end={{ x: 1, y: 1 }}
      ...
      <View style={styles.hero}>
        ...
        <View style={styles.info}>
          <View style={styles.headerRow}>
            ...
            <Text style={[styles.metaValue, { color }]}>{score.toLocaleString()} goals</Text>
          </View>
        ...
        <View style={styles.metaRow}>
          ...
          <View style={styles.metaCol}>
            ...
            <Text style={[styles.metaValue, { color }]}>
              {nextRank ? `${toNext.toLocaleString()} goals` : 'MAXED'}
            </Text>
          </View>
        </View>
